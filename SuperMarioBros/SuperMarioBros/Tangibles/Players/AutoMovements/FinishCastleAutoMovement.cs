...
{
    public class FinishCastleAutoMovement : IAutoMovement
    {
        ...
        public FinishCastleAutoMovement(IPlayer player)
        {
            this.IsActive = true;
            this.Player = player;
            this.moves = new List<Tuple<ICommand, int>>()
            {
                new Tuple<ICommand, int>(new GoRightPlayerCommand(Player), 120),
