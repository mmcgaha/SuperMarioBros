...
{
    public class GrassLedge : IBlock
    {
        ...
        public GrassLedge(int height, int width, int x = 0, int y = 0)
        {
            this.Position = new Vector2(x, y);
            this.Velocity = Vector2.Zero;
            this.Hitbox = new Hitbox(Hitboxes.BLOCK_OFFSET_X, Hitboxes.BLOCK_OFFSET_Y);
            this.State = new SLedgeIdle(this, width);
            this.Sprite = new GrassLedgeAnimation(Textures.grassLedge, width, height);
            this.IsActive = true;
