...
{
    public class SClimbingIdleRightFireMario : IFireMarioState, IClimbingMarioState, IRightMarioState
    {
        public IPlayer Player { get; set; }
        private int timer;

        public SClimbingIdleRightFireMario(IPlayer player)
        {
            this.Player = player;
            this.timer = 0;
            Player.Sprite = new MarioAnimation(HUD.currentPlayer == 0 ? Textures.mario0 : Textures.luigi0, Textures.fireRightClimbingIdle);
        ...
        public void GoDown()
        {
            Player.State = new SClimbingDownRightFireMario(Player);
        }
